pipeline {
    ...
    tools {
        jfrog 'jfrog-cli' // Ensuring JFrog CLI is available
    }
        ...
        stage('JFrog Testing') {
            steps {
                script {
                    ...
                    jf 'rt u test-file jfrog-cli-2/' // Upload test file to Artifactory
                    jf 'rt bp' // Build publish
                    jf 'rt dl jfrog-cli-2/test-file' // Download test file
                }
            }
        }
    }
    ...
    stages {
        ...
        stage('Checkout Code') {
            steps {
                git branch: 'main', url: 'https://github.com/Chamelio/aws-s3-bucket.git' 
            }
        }
